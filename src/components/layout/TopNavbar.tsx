...
export const TopNavbar: React.FC<{ onNavigateToCash?: () => void }> = ({ onNavigateToCash }) => {
  const {
    branches,
    currentBranch,
    setCurrentBranch,
  ...
  return (
    <header className="h-14 bg-white border-b border-emerald-100 px-4 flex items-center justify-between z-30 shrink-0 select-none shadow-sm">
      {/* Selector de Sucursal y Reloj */}
      <div className="flex items-center gap-4">
        {/* Branch Selector Dropdown */}
        <div className="relative">
          <button
            onClick={() => setShowBranchMenu(!showBranchMenu)}
            className="flex items-center gap-2 px-3 py-1.5 rounded-xl bg-emerald-50 hover:bg-emerald-100/80 text-xs font-bold text-emerald-900 border border-emerald-200 transition-all shadow-sm"
          >
      ...
      <div className="flex items-center gap-3">
        ...
        <div className="relative">
          ...
          {/* Drawer de Alertas */}
          {showAlerts && (
            <div className="absolute right-0 top-full mt-1.5 w-80 bg-white border border-emerald-100 rounded-2xl shadow-xl p-3 z-50">
        ...
        <div className="flex items-center gap-2 pl-2 border-l border-emerald-100">
          ...
          <button
            ...
            className="p-1.5 ml-1 rounded-xl bg-slate-100 hover:bg-red-50 text-slate-500 hover:text-red-600 border border-slate-200 transition-colors cursor-pointer"
          >
